...
export function sanitizeContent(content: string): string {
  ...
  content = redactSensitiveTokens(content);
  return content;
}

export function redactSensitiveTokens(content: string): string {
  // GitHub PAT tokens (keep for safety)
  content = content.replace(/\bgh[pors]_[A-Za-z0-9]{36}\b/g, "[REDACTED_TOKEN]");
  content = content.replace(/\bgithub_pat_[A-Za-z0-9_]{11,221}\b/g, "[REDACTED_TOKEN]");

  // Generic token patterns often used in Gitea / self-hosted systems
  content = content.replace(/\btoken\s+[A-Za-z0-9._-]{20,}\b/gi, "[REDACTED_TOKEN]");
